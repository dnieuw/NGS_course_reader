...
The workflow takes an input "sample_data" tabular file and based on this file it will process all samples in parallel and automatically.
...
This creates a new directory at the current location. Move the `sample_data.tsv` file to the results directory using the `mv` (move) command:

``` bash
mv sample_data.tsv results
```

::: {.callout-tip}
If you get a "file not found" error after changing your directory (`cd`) you may need to write the complete path of your sample_data.tsv file e.g `mv /home/david/Documents/sample_data.tsv results`
:::
...
- `{config}`: This is the `.tsv` file we have created previously.
